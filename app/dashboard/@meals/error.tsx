...
export default function MealsSlotError({
  error,
  unstable_retry,
}: {
  ...
  useEffect(() => {
    console.error("Meals slot render failed", {
      ...
      <div className="flex flex-col items-start gap-4">
        <div>
          ...
          <h3 className="text-xl font-semibold tracking-tight text-foreground">
            We couldn&apos;t load the meals panel.
          </h3>
          <p className="mt-2 text-sm leading-6 text-foreground/70">
            Please try again. If the issue keeps happening, use the error
            reference below to inspect the server logs.
          </p>
        </div>
        {error.digest ? (
          <p className="rounded-full border border-border bg-surface-elevated px-3 py-1 text-xs text-foreground/70">
