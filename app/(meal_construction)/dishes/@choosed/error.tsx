...
export default function ChoosedSlotError({
  error,
  unstable_retry,
}: {
  ...
  useEffect(() => {
    console.error("Choosed slot render failed", {
  ...
  return (
    <section className="relative w-full rounded-[28px] border border-danger/20 bg-surface/80 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.12)] backdrop-blur-xl">
      ...
      <h3 className="text-xl font-semibold tracking-tight text-foreground">
        We couldn&apos;t load the selected dish details.
      </h3>
      <p className="mt-2 text-sm leading-6 text-foreground/70">Please try again.</p>
      {error.digest ? (
